...
export default function SignIn({ setPage }) {
    ...
    function submitHandle(e) {
        if (e.code === "Enter" || e.type === 'click') {
            e.preventDefault();
            const response = signInValidation(input, captcha, setInputError, setCaptcha)
                ...
                <div className='login_inputs_inner_div'>
                    <div className='login_heading_and_inputs'>
                        ...
                        <div className='inputs'>
                            <input className={inputError.email ? 'error' : ''}
                                onFocus={() => setInputError((state) => ({ ...state, email: "" }))}
                                type="text" placeholder='Username / Email' name='email'
                                onChange={(event) => { inputHandle(event) }}
                                onKeyDown={submitHandle} autoFocus />
                            {inputError.email &&
                                <p>{inputError.email}</p>
                            }
                            <input className={inputError.password ? 'error' : ''}
                                onFocus={() => setInputError((state) => ({ ...state, password: "" }))}
