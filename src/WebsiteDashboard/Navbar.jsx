...
export const DashBoardNavbar = () => {
  ...
  const handleFilterDate = (date, field) => {
    const filterdata = filterData.filter((item) => {
      if (
        (field === "from" &&
          dayjs(item.submission_date).isSameOrAfter(dayjs(date))) ||
        (field === "to" &&
          dayjs(item.submission_date).isSameOrBefore(dayjs(date)))
      ) {
        return item;
      }
    });
    ...
    const getInternshipData = async () => {
      try {
        const res = await fetch(
          `http://localhost:8080/data?_page=1&_limit=${limit}`
        );
  ...
  const fetchSinglePageData = async (currentpage) => {
    const res = await fetch(
      `http://localhost:8080/data?_page=${currentpage}&_limit=${limit}`
    );
    const data = await res.json();
    return data;
  };
      ...
      <TabContext value={value}>
        ...
        <TabPanel value="1">
          ...
          <DashBoardInputs
            ...
            onStatusFilter={onFilterValueSelected}
          />
          <DashBoardDetails />
